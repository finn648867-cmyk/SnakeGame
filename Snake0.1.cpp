...
class Screen
{
   ...
   public:
    ...
    {
        // 检查边界喵
        if (pos.m_X < 0 || pos.m_X >= m_Width || pos.m_Y < 0 || pos.m_Y >= m_Height)
        {
            return;  // 超出范围就不设置喵
        }
    ...
    void DrawWord(Position pos, int type)
    {
        ...
        m_Buffer += "\033[" + std::to_string(ansi_y) + ';' + std::to_string(ansi_x) + 'H';
        switch (type)
        {
            case 0:  // 空地
    ...
    {
        m_Buffer += "\033[" + std::to_string(pos.m_Y) + ';' + std::to_string(pos.m_X) + 'H';
        m_Buffer += text;
    }
...
class Snake
{
   ...
   public:
    ...
    Position GetBodPosition(int i) const
    {
        return m_Body[i];
    }
    ...
    void SetHeadPosition(Position NewHead)
    {
        m_Body[0] = NewHead;
    }
    ...
    {
        // 检测输入方向和运动方向是否相反
        if (IsOpposite(NewDir, m_CurrentDirection))
            return;
        ...
        else
        {
            ...
            if (IsOpposite(NewDir, m_NextDirection))
                return;
...
class Game
{
   ...
   public:
    ...
    void UpdateScreen()  // 仅完成更新屏幕显示工作
    {
        m_Screen.Render();
        if (!m_IsGameOver)
        {
            std::cout << "\033[" << (m_Screen.GetHeight() + 1) << ";1HScore: \033[1;35m" << m_Score << " " << ColorlessWords::Food << " \033[0m";
        }
    }
    ...
    void DrawWalls()
    {
        ...
        {
            m_Screen.SetPixel({x, 0}, 1);
            m_Screen.SetPixel({x, m_Screen.GetHeight() - 1}, 1);
        }
        for (int y = 0; y < m_Screen.GetHeight(); y++)
        {
            m_Screen.SetPixel(Position(0, y), 1);
            m_Screen.SetPixel(Position(m_Screen.GetWidth() - 1, y), 1);
        }
    }

    void DrawSnakeHead()
    {
        m_Screen.SetPixel(m_Snake.GetHeadPosition(), 3);
    }

    void DrawSnakeBody()
    {
        for (int i = 1; i < m_Snake.GetBody().size(); i++)
            m_Screen.SetPixel(m_Snake.GetBodPosition(i), 4);
        if (m_Snake.GetBody().size() > 1)
            m_Screen.SetPixel(m_Snake.GetBody().back(), 5);
    }
    ...
    void DrawFood()
    {
        if (m_IfNeedDrawFood)
        {
            do
            {
                Position NewFood = {GetRandomNumber(0, m_Screen.GetWidth() - 1),
                                    GetRandomNumber(0, m_Screen.GetHeight() - 1)};
                m_Food.SetFoodPosition(NewFood);
            } while (m_Screen.GetCurPixel(m_Food.GetFoodPosition()));
            m_Screen.SetPixel(m_Food.GetFoodPosition(), 2);
            ...
            switch (Situation)
            {
                case 1:  // 即碰到墙壁
                    if (m_Snake.GetHeadPosition().m_X == 0)
                        m_Snake.SetHeadPosition({m_Screen.GetWidth() - 2, CurHead.m_Y});
                    if (m_Snake.GetHeadPosition().m_X == m_Screen.GetWidth() - 1)
                        m_Snake.SetHeadPosition({1, CurHead.m_Y});
                    if (m_Snake.GetHeadPosition().m_Y == 0)
                        m_Snake.SetHeadPosition({CurHead.m_X, m_Screen.GetHeight() - 2});
                    if (m_Snake.GetHeadPosition().m_Y == m_Screen.GetHeight() - 1)
                        m_Snake.SetHeadPosition({CurHead.m_X, 1});
                    IsChange    = true;
                    NeedRecheck = true;  // ⭐ 传送后需要重新检测喵
                    break;
                case 2:
                    m_Score = m_Snake.GetBody().size();
                    if (m_Score == ((m_Screen.GetWidth() - 2) * (m_Screen.GetHeight() - 2) - 1))
                    {
                        m_IsGameOver     = true;
    ...
    void GameOver()
    {
        ...
        std::cout << color << "║ " << PadString(title, contentWidth) << " ║\033[0m\n";

        // 原因行喵
        std::cout << "\033[" << (centerY + 2) << ";" << centerX << "H";
        std::cout << color << "║ " << PadString(GetGameOverReasonText(), contentWidth) << " ║\033[0m\n";
        ...
        // ⭐ 边框用color，内容用紫色喵
        std::cout << color << "║ \033[1;35m" << PadString(scoreLine, contentWidth) << "\033[0m" << color << "  ║\033[0m\n";
